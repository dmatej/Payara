...
public class PayaraMicroLoggingReconfigurationITest {
    ...
    private final PayaraServerContainer micro = new PayaraServerContainer("payara/micro:" + CFG_DAS.getVersion(), CFG_MIC) //
        ...
        .withCreateContainerCmdModifier(cmd -> {
            cmd.withHostName(CFG_MIC.getHost());
        })
        .withFileSystemBind(warFileOnHost.getAbsolutePath(), warFileInMicro.getAbsolutePath())
        .withCommand("--clustermode", "domain:hostname-das:4900",
            "--hzPublicAddress", CFG_MIC.getHost(),
            "--name", "MicroTest", "--deploy", warFileInMicro.getAbsolutePath(), "--contextRoot",
            "/logging")
    ...
    public static void createApplication() {
        ...
        warFileOnHost = new File(TEST_CFG.getFile("build.directory"),
            PayaraMicroLoggingReconfigurationITest.class.getSimpleName() + ".war");
        war.as(ZipExporter.class).exportTo(warFileOnHost, true);
        warFileInMicro = new File("/", warFileOnHost.getName());
    }
    ...
    @Test
    public void testSetLogLevels() throws Exception {
        das.asAdmin("list-hazelcast-cluster-members");
        // TODO assert
        ...
        final String microIpAddress = micro.getContainerInfo().getNetworkSettings().getNetworks().values().iterator()
            .next().getIpAddress();
